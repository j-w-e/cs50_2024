...
void sepia(int height, int width, RGBTRIPLE image[height][width])
{
    ...
    for (int i = 0; i < height; i++) {
        for (int j = 0; j < width; j++) {
            ...
            /* WORD tmp; */
            /* tmp = round( .393 * image[i][j].rgbtRed + .769 * image[i][j].rgbtGreen + .189 * image[i][j].rgbtBlue ); */
            /* image[i][j].rgbtRed = tmp > 255 ? 255 : tmp; */
            /* tmp = round( .349 * image[i][j].rgbtRed + .686 * image[i][j].rgbtGreen + .168 * image[i][j].rgbtBlue); */
            /* image[i][j].rgbtGreen = tmp > 255 ? 255 : tmp; */
            /* tmp = round( .272 * image[i][j].rgbtRed + .534 * image[i][j].rgbtGreen + .131 * image[i][j].rgbtBlue); */
